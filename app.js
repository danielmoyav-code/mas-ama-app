...
function App(){
  ...
  // 1. Control de sesión corregido
  if(!userSession) {
    return React.createElement(LoginScreen, {
      scriptUrl: syncConfig.url,
      onLogin: (userData) => {
        setSession(userData);
        DB.set('userSession', userData);
      }
    });
  }
  ...
  const isAdmin = currentUser?.rol === 'jefe' || currentUser?.rol === ROLES?.JEFE;
  ...
  async function handleSync(direction='push'){
    if(!syncConfig.url) { toast('⚠️ Configura la URL de sync primero'); return; }
    setSyncState({status:'syncing'});
    try {
      if(direction==='pull'){
        const r = await doPull(syncConfig.url, userSession);
        if(r.ok){
          if(r.patients?.length>0) { setPatients(r.patients); DB.set('patients', r.patients); }
          if(r.attendance?.length>0){
            const newLog={...attendanceLog};
            r.attendance.forEach(a=>{ newLog[`${a.date}||${a.taller}||${a.rut}`]=a.value; });
        ...
        } else { setSyncState({status:'error'}); toast('❌ Error al recibir datos'); }
      } else {
        const r = await doPush(syncConfig.url, userSession, { patients, attendanceLog, sessionLog });
        if(r.ok) {
          setSyncState({status:'ok',lastSync:new Date().toLocaleTimeString('es-CL',{hour:'2-digit',minute:'2-digit'})});
          toast('✅ Datos enviados');
        } else { setSyncState({status:'error'}); toast('❌ Error al enviar'); }
      }
    } catch(e) { setSyncState({status:'offline'}); toast('⚠️ Sin conexión'); }
  }

  function handleLogout() {
    if(window.confirm(`¿Cerrar sesión de ${currentUser?.nombre}?`)) {
      setSession(null);
      DB.del('userSession');
    }
  }
  ...
  const navItems=[
    ...
    ...(isAdmin ? [{id:'usuarios',icon:'👥',label:'Equipo'}] : []),
  ...
  return React.createElement('div',{id:'app'},
    React.createElement('div',{className:'top-bar'},
      ...
      !hasBack&&syncing&&React.createElement('div',{style:{width:18,height:18,borderRadius:'50%', border:'2px solid rgba(255,255,255,.3)',borderTopColor:'#fff', animation:'spin .7s linear infinite'}}),
      !hasBack&&!syncing&&syncConfig.url&&React.createElement('button',{ className:'top-icon-btn',onClick:()=>handleSync('push'),title:'Sincronizar'},'🔄'),
      !hasBack&&alertCount>0&& React.createElement('span',{className:'badge',onClick:()=>setView('alertas')},alertCount),
      !hasBack&&React.createElement('button',{ className:'top-icon-btn', onClick:handleLogout, title:'Cerrar sesión' },React.createElement('div',{style:{ width:28,height:28,borderRadius:'50%', background:currentUser?.color||'#2471A3', display:'flex',alignItems:'center',justifyContent:'center', fontSize:13,fontWeight:800,color:'#fff' }},currentUser?.nombre?.[0]||'?'))
    ),
    ...
    !hasData&&view!=='config'
      ?React.createElement('div',{className:'page',style:{textAlign:'center',paddingTop:50}},
          ...
          React.createElement('p',{style:{color:'#777',fontSize:15,marginBottom:24,lineHeight:1.5}}, 'Ve a Configuración e importa el Maestro Excel para comenzar.'),
          React.createElement('button',{className:'btn btn-primary', style:{maxWidth:280,margin:'0 auto'},onClick:()=>setView('config')}, '⚙️ Ir a Configuración'))
      :view==='inicio'   ?React.createElement(ViewInicio,{patients:visiblePatients,attendanceLog,onNav:setView,currentUser,autoSync:syncConfig,syncStatus:syncState?.status,lastSync:syncState?.lastSync,doSync:()=>handleSync('push')})
    ...
    React.createElement('nav',{className:'bottom-nav', style:{overflowX:'auto', justifyContent:'flex-start', padding:'0 10px'}},
      navItems.map(item=>React.createElement('button',{key:item.id, className:`nav-item ${view===item.id?'active':''}`,onClick:()=>setView(item.id), style:{minWidth:'60px', flexShrink:0}},
